...
class DuelingDQN(nn.Module):
    def __init__(self, w=3, h=3, n_outputs=9):
        ...
        self.f_conv = nn.Conv2d(1, 32, kernel_size=3)
    ...
    def forward(self, x):
        ...
        x1 = self.network1(feat)
        x2 = self.network2(feat)
        q = x1 + (x2-x2.mean())
...
class DQNDuelTrainer(object):
    ...
    def nn_init(self, env):
        ...
        self.policy = DuelingDQN(env.n_cols, env.n_rows,
                           env.n_cols*env.n_rows).to(self.device)
        self.target = DuelingDQN(env.n_cols, env.n_rows,
                           env.n_cols*env.n_rows).to(self.device)
...
if __name__ == "__main__":
    env = TicTacToe(4, 4, 3)
    trainer = DQNDuelTrainer(env, memory_size=5_000, batch_size=512)
